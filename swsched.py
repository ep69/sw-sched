...
class Solution:
    ...
    def __init__(self, solver):
                ...
                for c in range(len(courses)):
                    if solver.Value(src[(s,r,c)]):
                        ...
                        if len(Ts) == 2 and Ts[0] in teachers_follow:
                            Ts[0], Ts[1] = Ts[1], Ts[2]
                        text += f"{slots[s]}\t {rooms[r]}\t{'+'.join(Ts)}\t{courses[c]}\n"

        text += "\n"
        text += f"Teachers' utilization:\n"
        ...
        if PENALTY_OVERWORK > 0:
            text += f"Overwork: {sum([solver.Value(p) for p in penalties_overwork])}\n"
        if PENALTY_UNDERWORK > 0:
            text += f"Underwork: {sum([solver.Value(p) for p in penalties_underwork])}\n"
        text += f"Days: {sum([solver.Value(p) for p in penalties_days])}\n"
        text += f"Split: {sum([solver.Value(p) for p in penalties_split])}\n"
        text += f"Daypref_bad: {sum([solver.Value(p) for p in penalties_daypref_bad])}\n"
        text += f"Daypref_slight: {sum([solver.Value(p) for p in penalties_daypref_slight])}\n"
        text += f"Timepref_bad: {sum([solver.Value(p) for p in penalties_timepref_bad])}\n"
        text += f"Timepref_slight: {sum([solver.Value(p) for p in penalties_timepref_slight])}\n"
        #text += f"Notwithnew: {sum([solver.Value(p) for p in penalties_notwithnew])}\n"
        #text += f"Balboa: {sum([solver.Value(p) for p in penalties_balboa])}\n"
        text += f"Rent: {solver.Value(smalltaken)}\n"
        text += f"Free_koliste: {solver.Value(free_koliste)}\n"
        text += f"Balboa_closed: {sum([solver.Value(p) for p in penalties_balboa_closed])}\n"
        text += f"Total: {sum([solver.Value(p) for p in penalties])}\n"
        

class MPSP(cp_model.CpSolverSolutionCallback):
    ...
    #def on_solution_callback(self):
        ...
        print(f"Putting {self.ObjectiveValue()} to the queue")
        self.queue.put(self.ObjectiveValue())
...
def solve(queue):
    ...
    status = solver.SolveWithSolutionCallback(model, MPSP(queue))
...
if __name__ == "__main__":
    queue = SimpleQueue()
    p = Process(target=solve, args=(queue,))
    p.start()
    time.sleep(15)
